...
local function UpdateFOV()
    ...
    local success, errorMsg = pcall(function()
        if Circle then
            Circle.Visible = shared.Forbidden.PointLock.Reach_Visibility
            Circle.Radius = shared.Forbidden.PointLock.Reach
...
Mouse.KeyDown:Connect(function(Key)
    if Key:lower() == shared.Forbidden.Target.Keybind:lower() then
        if shared.Forbidden.PointLock.Enabled then
            if IsTargeting then
...
RunService.RenderStepped:Connect(function()
    if IsTargeting and TargetPlayer and TargetPlayer.Character then
        ...
        if BodyPart then
            ...
            if shared.Forbidden.PointLock.Aligner then
                local humanoid = TargetPlayer.Character:FindFirstChildOfClass("Humanoid")
                if humanoid then
                    local moveDirection = humanoid.MoveDirection
                    predictedPosition = BodyPart.Position + (moveDirection * shared.Forbidden.PointLock.VelocityCompensation)
                end
            else
                local targetVelocity = TargetPlayer.Character.HumanoidRootPart.Velocity
                predictedPosition = BodyPart.Position + (targetVelocity * shared.Forbidden.PointLock.VelocityCompensation)
            end
            
            if predictedPosition then
                local DesiredCFrame = CFrame.new(Camera.CFrame.Position, predictedPosition)

                if shared.Forbidden.PointLock.StabilizationEnabled then
                    Camera.CFrame = Camera.CFrame:Lerp(DesiredCFrame, shared.Forbidden.PointLock.Stabilization)
                else
...
local function getDynamicFov(targetPosition)
    ...
    if distance <= shared.Forbidden.Ranges.CloseDistance then
        return shared.Forbidden.Silent.BoudingSizes.CloseFovRadius
    elseif distance <= shared.Forbidden.Ranges.MidDistance then
        return shared.Forbidden.Silent.BoudingSizes.MidFovRadius
    elseif distance <= shared.Forbidden.Ranges.FarDistance then
        return shared.Forbidden.Silent.BoudingSizes.FarFovRadius
    else
        return shared.Forbidden.Silent.BoudingSizes.FarFovRadius  
    end
end
...
local function isPartInFovAndVisible(part)
    if not shared.Forbidden.PointLock.Enabled or not IsTargeting or not TargetPlayer then
        return false
    end
...
UserInputService.InputEnded:Connect(function(input, isProcessed)
    if input.KeyCode == Enum.KeyCode[shared.Forbidden.Target.Keybind:upper()] and PointLock.mode == "hold" then
